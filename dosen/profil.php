...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'profil') {
        db_query(
            'UPDATE dosen SET nama = ?, email = ?, no_hp = ?, alamat = ? WHERE user_id = ?',
            'ssssi',
            [
                trim($_POST['nama']),
                trim($_POST['email']),
        ...
        $fname = upload_profile_photo('dosen', $_FILES['foto'] ?? [], 'dosen_' . ($dosen['nidn'] ?? $user['id']));
        if ($fname) {
            db_query('UPDATE dosen SET foto = ? WHERE user_id = ?', 'si', [$fname, $user['id']]);
    ...
    redirect('dosen/profil.php');
}

$dosen = db_fetch_one('SELECT * FROM dosen WHERE user_id = ?', 'i', [$user['id']]);

render_layout_start('Profil Saya', 'dosen', dosen_menu('profil'));
?>
<div class="row g-3">
<div class="col-lg-5">
<div class="glass-card text-center">
    <img src="<?= e(user_avatar_url($user)) ?>" alt="Avatar" class="profile-avatar mb-3">
    <h5 class="fw-bold"><?= e($dosen['nama'] ?? $user['nama']) ?></h5>
    <p class="text-muted"><?= e($dosen['nidn'] ?? '') ?> · Dosen</p>
...
<div class="glass-card form-genz mb-3">
    ...
    <form method="post">
        ...
        <div class="mb-2"><label>NIDN</label><input class="form-control" value="<?= e($dosen['nidn'] ?? '') ?>" readonly></div>
        <div class="mb-2"><label>Nama</label><input name="nama" class="form-control" value="<?= e($dosen['nama'] ?? '') ?>" required></div>
        <div class="mb-2"><label>Email</label><input name="email" type="email" class="form-control" value="<?= e($dosen['email'] ?? '') ?>"></div>
        <div class="mb-2"><label>No. HP</label><input name="no_hp" class="form-control" value="<?= e($dosen['no_hp'] ?? '') ?>"></div>
        <div class="mb-2"><label>Alamat</label><textarea name="alamat" class="form-control" rows="2"><?= e($dosen['alamat'] ?? '') ?></textarea></div>
