...
$user = current_user();
$admin = db_fetch_one('SELECT * FROM admin WHERE user_id = ?', 'i', [$user['id']]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'profil') {
        db_query(
            'UPDATE admin SET nama = ?, email = ?, no_hp = ?, alamat = ? WHERE user_id = ?',
            'ssssi',
            [
                trim($_POST['nama']),
                trim($_POST['email']),
        ...
        $fname = upload_profile_photo('admin', $_FILES['foto'] ?? [], 'admin_' . $user['id']);
        if ($fname) {
            db_query('UPDATE admin SET foto = ? WHERE user_id = ?', 'si', [$fname, $user['id']]);
...
render_layout_start('Profil Saya', 'admin', admin_menu('profil'));
?>
<div class="row g-3">
<div class="col-lg-5">
<div class="glass-card text-center">
    <img src="<?= e(user_avatar_url($user)) ?>" alt="Avatar" class="profile-avatar mb-3">
    <h5 class="fw-bold"><?= e($admin['nama'] ?? $user['nama']) ?></h5>
    <p class="text-muted"><?= e($user['username']) ?> · Administrator</p>
...
<div class="glass-card form-genz mb-3">
    ...
    <form method="post">
        ...
        <div class="mb-2"><label>Nama</label><input name="nama" class="form-control" value="<?= e($admin['nama'] ?? '') ?>" required></div>
        <div class="mb-2"><label>Email</label><input name="email" type="email" class="form-control" value="<?= e($admin['email'] ?? '') ?>"></div>
        <div class="mb-2"><label>No. HP</label><input name="no_hp" class="form-control" value="<?= e($admin['no_hp'] ?? '') ?>"></div>
        <div class="mb-2"><label>Alamat</label><textarea name="alamat" class="form-control" rows="2"><?= e($admin['alamat'] ?? '') ?></textarea></div>
